...
namespace Comfy.Data.Core
{
    ...
    {
        public static char[] PrefixCharArray = new char[] { '@', ':', '?' };
        ...
        public static void AddParameters(Dictionary<string, KeyValuePair<DbType, object>> targetParameters, IExpression hp)
        {
            ...
            while (en.MoveNext())
            {
                ...
                targetParameters.Add('@' + en.Current.Key.TrimStart(SqlQueryUtils.PrefixCharArray), en.Current.Value);
            }
        }
        ...
        public static void AppendColumnName(StringBuilder sb, string columnName)
        {
            if (columnName.Contains("."))
            {
                ...
                {
                    if (splittedColumnSections[i].Contains("*") 
                        || splittedColumnSections[i].Contains("(") 
                        || splittedColumnSections[i].Contains("["))
                    {
        ...
        public static string RemoveTableAliasNamePrefixes(string sql)
        {
            System.Text.RegularExpressions.Regex r = 
                new System.Text.RegularExpressions.Regex(@"\[([\w\d\s_]+)\].\[([\w\d\s_]+)\]");
            sql = r.Replace(sql, "[$2]");
            return sql;
        }
    }
