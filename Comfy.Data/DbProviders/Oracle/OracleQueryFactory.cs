...
namespace Comfy.Data.DbProviders.Oracle
{
    public class OracleQueryFactory : SqlQueryFactory
    {
        ...
        protected override void AddInputParametersToCommand(string[] paramNames, DbType[] paramTypes, object[] paramValues, DbCommand cmd)
        {
            if (paramNames != null)
            {
                for (int i = 0; i < paramNames.Length; ++i)
                {
                    ...
                    p.ParameterName = paramNames[i].TrimStart(paramPrefixToken);
        ...
        protected override void AddOutputParametersToCommand(string[] paramNames, DbType[] paramTypes, int[] paramSizes, DbCommand cmd)
        {
            if (paramNames != null)
            {
                for (int i = 0; i < paramNames.Length; ++i)
        ...
        protected override void AddInputOutputParametersToCommand(string[] paramNames, DbType[] paramTypes, int[] paramSizes, object[] paramValues, DbCommand cmd)
        {
            if (paramNames != null)
            {
                for (int i = 0; i < paramNames.Length; ++i)
        ...
        protected override void AddReturnValueParameterToCommand(string paramName, DbType paramType, int paramSize, DbCommand cmd)
        {
            if (!string.IsNullOrEmpty(paramName))
            {
                DbParameter p = cmd.CreateParameter();
                p.ParameterName = paramName.TrimStart(paramPrefixToken);
